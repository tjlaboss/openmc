...
class MGXS(object):
    ...
    def get_mgxs(mgxs_type, domain=None, domain_type=None,
        ...
        Parameters
        ----------
        mgxs_type : {'total', 'transport', 'nu-transport', 'diffusion-coefficient', 'absorption', 'capture', 'fission', 'nu-fission', 'kappa-fission', 'scatter', 'nu-scatter', 'scatter matrix', 'nu-scatter matrix', 'multiplicity matrix', 'nu-fission matrix', 'chi', 'chi-prompt', 'inverse-velocity', 'prompt-nu-fission', 'prompt-nu-fission matrix'}
...
class DiffusionCoefficient(MGXS):
    r"""A transport-corrected total multi-group cross section.
    ...
    def xs_tally(self):
        if self._xs_tally is None:
            ...
            self._xs_tally = self.rxn_rate_tally / self.tallies['flux']
            self._xs_tally = 1.0 / 3.0 * self._xs_tally**(-1)
            self._compute_xs()
    ...
    def rxn_rate_tally(self):
        if self._rxn_rate_tally is None:
            ...
            self._rxn_rate_tally = self.tallies['total'] - self.tallies['scatter-1']
            self._rxn_rate_tally.sparse = self.sparse
    ...
    def get_condensed_xs(self, coarse_groups):
        ...
        # Get the tallies that should be collapsed
        rxn_tally = condensed_xs.xs_tally * condensed_xs.tallies['flux']
        tallies = [condensed_xs.tallies['flux'], rxn_tally]
        ...
        # Compute the energy condensed multi-group cross section
        condensed_xs.sparse = self.sparse
        condensed_xs.tallies['flux'] = tallies[0]
        condensed_xs._rxn_rate_tally = tallies[1]
        condensed_xs._xs_tally = tallies[1] / tallies[0]
