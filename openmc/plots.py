...
class PlotBase(IDManagerMixin):
    ...
    def colorize(self, geometry, seed=1):
        ...
        Else, if a cell/material has a color assigned, its color will be used.
    ...
    def to_xml_element(self):
        ...
        if self._background is not None:
            ...
            if isinstance(color, str):
                color = openmc.colors.SVG_COLORS[color.lower()]
...
class Plot(PlotBase):
    ...
    def highlight_domains(self, geometry, domains, seed=1,
        ...
        if isinstance(background, str):
            if background.lower() not in openmc.colors.SVG_COLORS:
                raise ValueError(f"'{background}' is not a valid color.")
            background = openmc.colors.SVG_COLORS[background.lower()]
...
class ProjectionPlot(PlotBase):
    ...
    def __init__(self, plot_id=None, name=''):
        ...
        self._wireframe_color = openmc.colors.SVG_COLORS['black']
